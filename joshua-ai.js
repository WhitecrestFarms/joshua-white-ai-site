(() => {
  ...
  const responses = [
    {
      ...
      actions: [
        ...
        ["Email Joshua", `mailto:${EMAIL}?subject=Home Value Request`],
    ...
    },
    {
      ...
      actions: [
        ...
        ["Email Joshua", `mailto:${EMAIL}?subject=Buyer Question`]
      ]
    },
    {
      ...
      actions: [
        ["Start Mortgage Application", LINKS.mortgage],
        ["Email Joshua", `mailto:${EMAIL}?subject=Mortgage Question`]
      ]
    },
    {
      ...
      actions: [
        ...
        ["Office Listings", "#office-listings"]
      ]
    },
    {
      ...
      actions: [
        ["Google Reviews", LINKS.google],
        ["Zillow Profile", LINKS.zillow],
    ...
    },
    {
      ...
      actions: [
        ["Call/Text Joshua", PHONE_LINK],
        ["Email Joshua", `mailto:${EMAIL}?subject=Website Inquiry`],
    ...
    },
    {
      ...
      actions: [
        ["Search Homes", LINKS.search],
        ["Ask Joshua", `mailto:${EMAIL}?subject=Area Question`]
      ]
    },
    {
  ...
  function createStyles() {
    const css = `
      .jw-ai-launcher{position:fixed;right:22px;bottom:22px;z-index:9999;border:0;border-radius:999px;background:#b88128;color:#fff;padding:15px 20px;font-weight:900;box-shadow:0 18px 44px rgba(14,58,50,.28);cursor:pointer;font-size:16px;display:flex;gap:9px;align-items:center}
      .jw-ai-launcher:hover{transform:translateY(-2px)}
      .jw-ai-window{position:fixed;right:22px;bottom:86px;z-index:9999;width:min(420px,calc(100vw - 28px));height:min(640px,calc(100vh - 120px));background:#fff;border:1px solid rgba(184,161,113,.45);border-radius:24px;box-shadow:0 24px 80px rgba(14,58,50,.28);overflow:hidden;display:none;flex-direction:column;font-family:Arial,Helvetica,sans-serif}
      .jw-ai-window.open{display:flex}
      .jw-ai-header{background:linear-gradient(135deg,#0e3a32,#005a4e);color:#fff;padding:18px 18px 15px;display:flex;justify-content:space-between;gap:12px;align-items:flex-start}
      .jw-ai-header strong{display:block;font-size:20px}.jw-ai-header span{display:block;color:#dbcfa7;font-size:13px;margin-top:3px}.jw-ai-close{background:transparent;color:#fff;border:1px solid rgba(255,255,255,.45);border-radius:999px;width:34px;height:34px;cursor:pointer;font-size:20px}
      .jw-ai-messages{padding:16px;overflow:auto;display:flex;flex-direction:column;gap:12px;flex:1;background:linear-gradient(180deg,#fff,#ece9d3)}
      .jw-msg{max-width:88%;padding:12px 14px;border-radius:18px;font-size:14px;line-height:1.45;white-space:pre-wrap}.jw-bot{background:#fff;border:1px solid rgba(184,161,113,.36);color:#2e2e2e;align-self:flex-start}.jw-user{background:#0e3a32;color:#fff;align-self:flex-end}
      .jw-actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:8px}.jw-actions a,.jw-chip{border:1px solid rgba(14,58,50,.18);background:#d0e2da;color:#0e3a32;border-radius:999px;padding:8px 10px;font-weight:800;font-size:12px;text-decoration:none;cursor:pointer}.jw-actions a:hover,.jw-chip:hover{background:#9fc8bd}
      .jw-quick{padding:12px 14px;border-top:1px solid rgba(184,161,113,.30);display:flex;gap:8px;flex-wrap:wrap;background:#fff}
  ...
  function buildWidget() {
    ...
    windowEl.innerHTML = `
      <div class="jw-ai-header">
        <div><strong>Joshua AI</strong><span>Real estate assistant for Joshua White</span></div>
        <button class="jw-ai-close" aria-label="Close Joshua AI">×</button>
      </div>
      ...
      <form class="jw-ai-form">
        <input class="jw-ai-input" type="text" placeholder="Ask about buying, selling, homes, or value..." />
        <button class="jw-ai-send" type="submit">Send</button>
      </form>
    `;
    ...
    addMessage(messages, "bot", `Hi, I'm Joshua AI — Joshua White's digital real estate assistant. I can help with home search, home values, buying, selling, mortgage options, reviews, or connecting you with Joshua. What can I help with today?`, [
      ["Search Homes", LINKS.search],
      ["Home Value", LINKS.value],
      ["Contact Joshua", "#contact"]
    ]);
    ...
    windowEl.querySelectorAll(".jw-chip").forEach(chip => {
